...
import { ArrowLeft, Star, Share2, PenLine, Loader2, ChevronUp, Wallet, Heart } from 'lucide-react';
import DishCard from '../components/DishCard';
import { Dish, Review, Restaurant } from '../types';
...
const RestaurantDetail: React.FC = () => {
  ...
  // 2. Fetch Reviews (Separately for performance simulation)
  useEffect(() => {
    window.scrollTo(0, 0);
    if (id) {
  ...
  const averagePrice = useMemo(() => {
    if (!restaurant || !restaurant.menu || restaurant.menu.length === 0) return 0;
        ...
        {activeTab === 'menu' ? (
             <div className="space-y-1">
                {(!restaurant.menu || restaurant.menu.length === 0) ? (
                    <div className="text-center py-8 text-gray-400 text-sm">暂无菜品</div>
                ) : (
      ...
      <div className="fixed bottom-20 left-4 right-4 z-40">
        {hasCartItems ? (
          <div className="bg-black text-white p-2 pl-4 pr-2 rounded-xl shadow-2xl flex justify-between items-center animate-in slide-in-from-bottom-5">
             ...
             >
                {isOrdering ? <Loader2 size={16} className="animate-spin mr-2" /> : '去结算'}
             </button>
          </div>
        ) : (
          <button 
