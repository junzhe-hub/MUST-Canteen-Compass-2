...
import { Home, LayoutGrid, Compass, User, Trophy } from 'lucide-react';
...
const BottomNav: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 pb-safe pt-2 px-6 flex justify-between items-center z-50 h-16 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
      ...
      {/* Main Action Button - Explore */}
      <div className="relative -top-5">
        <button 
          onClick={() => navigate('/explore')}
          className={`bg-black text-white p-4 rounded-full shadow-lg border-4 border-gray-100 flex items-center justify-center transform active:scale-95 transition-all ${isActive('/explore') ? 'ring-2 ring-yellow-400' : ''}`}
        >
